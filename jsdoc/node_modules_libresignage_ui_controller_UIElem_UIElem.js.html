...
<head>
    <meta charset="utf-8">
    <title>JSDoc: Source: node_modules/libresignage/ui/controller/UIElem/UIElem.js</title>
...
<div id="main">
    ...
    <h1 class="page-title">Source: node_modules/libresignage/ui/controller/UIElem/UIElem.js</h1>
...
class UIElem {
	...
	*/
	constructor(args) {
		Assert.ok('elem' in args);
		Assert.ok('cond' in args);
		Assert.ok('enabler' in args);
		Assert.ok('attach' in args);
		Assert.ok('defer' in args);
		...
		for (let key in this.attach) {
			this.get_elem().addEventListener(key, (...evargs) => {
				let ret = null;
				if (
					(evargs[0].isTrusted &amp;&amp; this.is_event_running())
					|| this.defer()
				) {
					...
					ret = this.attach[key].apply(null, evargs);
	...
	/**
	* Get the HTML element this UIElem controls.
	*
