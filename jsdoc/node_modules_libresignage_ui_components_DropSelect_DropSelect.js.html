...
<head>
    <meta charset="utf-8">
    <title>JSDoc: Source: node_modules/libresignage/ui/components/DropSelect/DropSelect.js</title>
...
<div id="main">
    ...
    <h1 class="page-title">Source: node_modules/libresignage/ui/components/DropSelect/DropSelect.js</h1>
...
class DropSelect {
	...
	*
	* @throws {AssertionError} If no options are set.
	* @throws {AssertionError} If the requested option doesn't exist.
	*/
	select(option, dispatch_event) {
		...
		for (let e of elems) {
			if (e.getAttribute('value') === option) {
				e.classList.add('dropselect-selected');
			} else {
				e.classList.remove('dropselect-selected');
			}
		}
		if (dispatch_event) {
			this.container.dispatchEvent(
